...
uses System.SysUtils, System.Classes, math, Windows, Messages, System.Types;

const
     MaxPopSize = 1000;//High(Word); // 65 535
     MaxChromoSize = 100;//High(Byte);   // 255
     RndArraySize = 55;
...
TGAlgorithm = class(TComponent)
  ...
  protected
    ...
    procedure qkSort(var APop: TPopulation; LimitLow,  LimitHigh: TLimitPop);


    procedure NextGenerationRnd;
    procedure StartGenerationRnd(Rand:Real);
    function FRandom:Real;
    function FFaceCrown(Probability:Real):boolean;
    function FHigh(min,max:integer):integer;
    function FSeed(ValueSeed:Real):boolean;
    procedure StartingPopulation(var Pop:TPopulation);

public
    constructor Create(AOwner: TComponent);override;
    function Initialized: Boolean;

    function BinToInt(Chromo:TChromosome; LowCrom, HighChromo: integer):longInt;
    ...
    function Mutation(var AGenotype:TGenotype):boolean;
    procedure Start;
...
function TGAlgorithm.BinToInt(Chromo: TChromosome; LowCrom, HighChromo: integer): longInt;
var
  i:integer;
...
if (VarChromoSize > MaxChromoSize) or (LowCrom>HighChromo) then BinToInt:=-1
    ...
    begin
    for i:=HighChromo downto LowCrom do
        begin
...
function TGAlgorithm.Mutation(var AGenotype: TGenotype): boolean;
var
sorteado: integer;
begin
if FFaceCrown(pbMutation) then
   begin
     ...
     AGenotype.Chromo[sorteado]:=not(AGenotype.Chromo[sorteado]);
   AGenotype.Mutation:=sorteado;
   Mutation:=true;
   end
else Mutation:=false;
end;
...
procedure TGAlgorithm.qkSort(var APop: TPopulation; LimitLow,
  LimitHigh: TLimitPop);
var
esq,dir:TLimitPop;
pivo,aux:TGenotype;
...
esq:=LimitLow;
dir:=LimitHigh;
pivo:=APop.Genotypes[(LimitLow+LimitHigh) div 2];

while(esq <= dir) do
   begin
   if VarOptimizationType=otMaximum then
      begin
      while (APop.Genotypes[esq].FitnessValue > pivo.FitnessValue) do inc(esq);
      while (APop.Genotypes[dir].FitnessValue < pivo.FitnessValue) do dec(dir);
      end
   else
       begin
       while (APop.Genotypes[esq].FitnessValue < pivo.FitnessValue) do inc(esq);
   ...
   if (esq <= dir) then
      begin
      aux:=APop.Genotypes[esq];
      APop.Genotypes[esq]:=APop.Genotypes[dir];
      APop.Genotypes[dir]:=aux;
      inc(esq);
      dec(dir);
      end;
    end;

if (dir-LimitLow)>0 then qkSort(APop,LimitLow,dir);
if (LimitHigh-esq)>0 then qkSort(APop,esq,LimitHigh);
...
case VarSelectionType of
     stRoulette:
            ...
            repeat
               inc(indPop);
               SumTemp:=SumTemp+populacao.Genotypes[indPop].FitnessValue;
               until (SumTemp>=Rand) or (indPop=VarPopSize);
            Select:=indPop;
            end;
     stRank:
          begin
